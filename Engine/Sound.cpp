...
namespace Sound
{
    ...
    void Import(std::string const& file)
    {
        ...
        handle->buffer.Flags = XAUDIO2_END_OF_STREAM;   // 해당버퍼의 오디오는 변경불가하게 설정
	...
	void Procedure(HWND const hWindow, UINT const uMessage, WPARAM const wParameter, LPARAM const lParameter)
	{
		switch (uMessage)
		{
			case WM_CREATE:
			{
				MUST(CoInitializeEx(nullptr, COINITBASE_MULTITHREADED));     // XAudio2를 사용하기위해 활성화 시켜줌
    ...
    void Sound::Play()
    {
        ...
        if (loop) sound->buffer.LoopCount = XAUDIO2_LOOP_INFINITE;      // 반복재생 (횟수설정 가능)
