...
namespace Input
{
	...
	public :
		void Down(WPARAM const code)
		{
			if (!Pressed(code))
			{
				...
				State.OnceDown[code >> 0x4] ^= (0x8000 >> (code & 0xf));
			}
		}
		...
		{
			if (Pressed(code))
				State.OnceUp[code >> 0x4] ^= (0x8000 >> (code & 0xf));
			State.Pressed[code >> 0x4] ^= (0x8000 >> (code & 0xf));
			State.Changed[code >> 0x4] ^= (0x8000 >> (code & 0xf));
		}
		...
		bool OnceUp(WPARAM const code)
		{
			if (State.OnceUp[code >> 0x4] & (0x8000 >> (code & 0xf))) {
				State.OnceUp[code >> 0x4] ^= (0x8000 >> (code & 0xf));
				printf("OnceUp\r\n");
				return true;
			}
		...
		bool OnceDown(WPARAM const code)
		{
			if (State.OnceDown[code >> 0x4] & (0x8000 >> (code & 0xf))) {
				State.OnceDown[code >> 0x4] ^= (0x8000 >> (code & 0xf));
				printf("OnceDown\r\n");
				return true;
			}
	...
	namespace Get
	{
		namespace Key
		{
			...
			bool Changed(size_t const code) { return Input::Key.Changed(code); }
			bool OnceUp(size_t const code) { return Input::Key.OnceUp(code); }
			bool OnceDown(size_t const code) { return Input::Key.OnceDown(code); }
		}
